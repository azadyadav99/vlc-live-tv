...
| Key | Action |
|-----|--------|
| n (or top-row/numpad 6) | Next channel |
| p (or top-row/numpad 4) | Previous channel |
...
## Channel Checker (VLC extension)

`vlc\ChannelChecker.lua` (installed to `lua\extensions`) adds an **Extensions > Channel Checker** menu inside VLC:

- **All Channels**: browse the playlist by group (Sony Sab first), play the selected channel.
- **Check All Channels**: probes streams and writes `channel_check_results.txt` to your VLC data folder. A "Check first" limit (default 200) keeps VLC usable; VLC stays busy during a check.
- **Show All Channel Names**: exports a numbered `channel_names.txt`.

If the Extensions menu is missing, make sure VLC loads Lua extensions (the installer sets `qt-autoload-extensions=1` in vlcrc).

## Edit channels

- Edit `playlist\livetv.m3u8` (`#EXTINF` line + URL line)
- `menu\ChannelMenu.html` is GENERATED by `install\build-menu.ps1` - edit the generator (or the playlist) and regenerate; do not hand-edit the HTML
